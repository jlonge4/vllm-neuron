...
# =============================================================================
# Section 2: Rotary Position Embedding (standard RoPE)
# =============================================================================
class MiniMaxM2RotaryEmbedding(nn.Module):
    """Partial Rotary Position Embedding — only rotary_dim of head_dim gets RoPE."""
    ...
    def forward(
        ...
        inv_freq_expanded = self.inv_freq[None, :].float()  # [1, rotary_dim/2]
        position_ids_expanded = position_ids[:, None].float()  # [T, 1]

        freqs = position_ids_expanded @ inv_freq_expanded  # [T, rotary_dim/2]
        cos = freqs.cos()
        sin = freqs.sin()
...
class MiniMaxM2Attention(nn.Module):
    ...
    def forward_prefill(
        ...
        # MiniMaxM2RotaryEmbedding emits cos/sin of shape [T, d_head/2]; the
...
class MiniMaxM2Experts(nn.Module):
    ...
    def __init__(self, config: MiniMaxM2Config):
        ...
        self.router_bias = nn.Parameter(
            torch.empty(self.total_num_experts, dtype=torch.bfloat16)
        )
...
class MiniMaxM2DecoderLayer(nn.Module):
    ...
    def __init__(self, config: MiniMaxM2Config, batch_size: int, layer_idx: int):
        super().__init__()
        # <-- MODEL-SPECIFIC: Pre-attention RMSNorm
        self.input_layernorm = MiniMaxM2RMSNorm(config)
        self.self_attn = MiniMaxM2Attention(config, layer_idx=layer_idx)
        self.mlp = MiniMaxM2MLP(config)
        self.layer_idx = layer_idx
...
class MiniMaxM2Model(nn.Module):
    ...
    def __init__(self, config: MiniMaxM2Config, batch_size: int):
        super().__init__()
        self.config = config
        ...
        self.layers = nn.ModuleList(
            [
                MiniMaxM2DecoderLayer(config, batch_size, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )

        # <-- MODEL-SPECIFIC: Final RMSNorm
        self.norm = MiniMaxM2RMSNorm(config)
        self.rotary_emb = MiniMaxM2RotaryEmbedding(config)
...
class MiniMaxM2ForCausalLM(nn.Module, SupportsEagle3):
    ...
    def __init__(self, config: MiniMaxM2Config, batch_size: int):
        super().__init__()
        self.config = config
        self.model = MiniMaxM2Model(config, batch_size)
    ...
    def from_configs(cls, hf_config: PretrainedConfig, neuron_config: NeuronConfig):
        config = MiniMaxM2Config.from_configs(hf_config, neuron_config)
    ...
    def load_weights(
        ...
        for layer_id in range(len(self.model.layers)):
            ...
            # Attention weight mappings — FP8 with scale_inv
            mappings[f"{layer_prefix}.self_attn.qkv_proj_weight"] = [
                f"{layer_prefix}.self_attn.q_proj.weight",
                f"{layer_prefix}.self_attn.q_proj.weight_scale_inv",
                f"{layer_prefix}.self_attn.k_proj.weight",
                f"{layer_prefix}.self_attn.k_proj.weight_scale_inv",
                f"{layer_prefix}.self_attn.v_proj.weight",
                f"{layer_prefix}.self_attn.v_proj.weight_scale_inv",
            ]
            ...
            mappings[f"{layer_prefix}.self_attn.o_proj_weight"] = [
                ...
                f"{layer_prefix}.self_attn.o_proj.weight_scale_inv",
            ]
            ...
            mappings[f"{layer_prefix}.mlp.experts.router_bias"] = (
                f"{moe_prefix}.e_score_correction_bias"
            )
            # Expert weights: w1=gate, w3=up, w2=down — FP8 with scale_inv
            num_experts = self.config.num_experts
            # gate_up: [w1_0_weight, w1_0_scale, ..., w3_0_weight, w3_0_scale, ...]
            gate_keys = []
            for e in range(num_experts):
                gate_keys.append(f"{moe_prefix}.experts.{e}.w1.weight")
                gate_keys.append(f"{moe_prefix}.experts.{e}.w1.weight_scale_inv")
            up_keys = []
            for e in range(num_experts):
                up_keys.append(f"{moe_prefix}.experts.{e}.w3.weight")
                up_keys.append(f"{moe_prefix}.experts.{e}.w3.weight_scale_inv")
            mappings[f"{layer_prefix}.mlp.experts.gate_up_proj_weight"] = (
                gate_keys + up_keys
            )
            # down: [w2_0_weight, w2_0_scale, ...]
            down_keys = []
            for e in range(num_experts):
                down_keys.append(f"{moe_prefix}.experts.{e}.w2.weight")
                down_keys.append(f"{moe_prefix}.experts.{e}.w2.weight_scale_inv")
            mappings[f"{layer_prefix}.mlp.experts.down_proj_weight"] = down_keys
